...
import 'package:memberlike/models/request/message_request.dart';
import 'package:memberlike/models/request/ticket_request.dart';
import 'package:memberlike/models/response/ticket_chat_response.dart';
...
class TicketController extends GetxController {
  ...
  Future<bool> getMessages() async {
    final res =
        await apiRepository.getChats(selectedTicket.value!.id.toString());
    if (res != null) {
      for (int i = 0; i < res.data.length; i++) {
        ...
        if (messages.any((element) =>
            element.id == int.parse(res.data[i]["id"].toString()))) {
        } else {
  ...
  Future sendMessage() async {
    if (messageController.text.trim() != "") {
      ...
      if (res != null) {
        messages.add(TicketChatResponse(
