...
import 'country_dialog_custom.dart';
...
class IntlPhoneFieldCustom extends StatefulWidget {
  ...
  final List<CountryCustom>? countries;
  ...
  /// & pick dialog
  final CountryCustomPickerDialog? pickerDialogStyleCustom;
  ...
  const IntlPhoneFieldCustom({
    ...
    @Deprecated(
      'Use searchFieldInputDecoration of PickerDialogStyleCustom instead',
    )
    ...
    this.pickerDialogStyleCustom,
    this.flagsButtonMargin = EdgeInsets.zero,
  });

  @override
  _IntlPhoneFieldCustomState createState() => _IntlPhoneFieldCustomState();
}

class _IntlPhoneFieldCustomState extends State<IntlPhoneFieldCustom> {
  late List<CountryCustom> _countryList;
  late CountryCustom _selectedCountry;
  late List<CountryCustom> filteredCountries;
  late String number;
  ...
  void initState() {
    ...
    if (widget.initialCountryCode == null && number.startsWith('+')) {
      ...
      _selectedCountry = countries.firstWhere(
        (country) => number.startsWith(country.fullCountryCustomCode),
        orElse: () => _countryList.first,
      );

      // remove country code from the initial number value
      number = number.replaceFirst(
        RegExp("^${_selectedCountry.fullCountryCustomCode}"),
        "",
      );
    } else {
    ...
    await showDialog(
      ...
      builder:
          (context) => StatefulBuilder(
            builder:
                (ctx, setState) => CountryCustomPickerDialog(
                  languageCode: widget.languageCode.toLowerCase(),
                  // style: widget.pickerDialogStyleCustom,
                  filteredCountries: filteredCountries,
                  searchText: widget.searchText,
                  countryList: _countryList,
                  selectedCountry: _selectedCountry,
                  onCountryChanged: (CountryCustom country) {
    ...
    return TextFormField(
      ...
      onChanged: (value) async {
        final phoneNumber = PhoneNumber(
          countryISOCode: _selectedCountry.code,
          countryCode: '+${_selectedCountry.fullCountryCustomCode}',
          number: value,
        );
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                Padding(
                  padding: getPadding(bottom: 1, right: 6),
                  child: FittedBox(
